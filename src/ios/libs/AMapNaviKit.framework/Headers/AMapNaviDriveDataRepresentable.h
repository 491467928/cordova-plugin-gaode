...
/**
 * @brief 巡航状态下的拥堵区域信息, 当拥堵长度大于500米且拥堵时间大于5分钟时回调, 当前方无拥堵信息时，回调的congestionInfo对象属性值都为空. 该更新回调只有在detectedMode开启后有效 since 6.7.0
 * @param driveManager 驾车导航管理类
 * @param congestionInfo 巡航信息,参考 AMapNaviCruiseCongestionInfo 类
 */
...
/**
 * @brief 巡航时需要显示车道信息时的回调.(使用方法可参考 driveManager: showLaneBackInfo: laneSelectInfo:方法注释). 该更新回调只有在detectedMode开启后有效  since 6.7.0
 * @param driveManager 驾车导航管理类
 * @param laneBackInfo 车道背景信息，例如：@"1|0|0|4"，表示当前道路有4个车道，分别为"左转|直行|直行|右转+直行"
 * @param laneSelectInfo 车道前景信息，其个数一定和车道背景信息一样，例如：@"255|0|0|0"，表示选择了当前道路的第2、3、4三个直行车道。
 */
- (void)driveManager:(AMapNaviDriveManager *)driveManager showCruiseLaneWithLaneBackInfo:(NSString *)laneBackInfo laneSelectInfo:(NSString *)laneSelectInfo;

/*
 * @brief 巡航需要隐藏车道信息时的回调. 该更新回调只有在detectedMode开启后有效 since 6.7.0
 * @param driveManager 驾车导航管理类
 */
- (void)driveManagerHideCruiseLane:(AMapNaviDriveManager *)driveManager;
